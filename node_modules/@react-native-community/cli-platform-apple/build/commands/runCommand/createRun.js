...
const createRun = ({
  ...
  if (!args.device && !args.udid && !args.simulator) {
    ...
    if (bootedSimulators.length === 0) {
      _cliTools().logger.info('No booted simulators found. Launching first available simulator...');
    } else {
      _cliTools().logger.info(`Found booted ${targetSimulator.name}`);
    }
    if (connectedDevices.length > 0) {
      _cliTools().logger.info(`Ignoring connected ${connectedDevices.length === 1 ? 'device' : 'devices'} (${connectedDevices.map(({
        name
      }) => name).join(', ')}). Pass --device or --udid to install on a physical device.`);
    }
    return (0, _runOnSimulator.runOnSimulator)(xcodeProject, platformName, mode, scheme, args, targetSimulator);
  }
  if (args.device && args.udid) {
